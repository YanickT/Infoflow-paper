...
if __name__ == "__main__":
    ...
    # initialize networks
    net = Network(784, 10, 10, [1.7, 0.05], device=device)
    conet = ContraNetwork(net, device=device)

    train_data, test_data = get_train_data()
    loss, accs = net.training(train_data, test_data, 1)
    loss, acc = net.eval(test_data)
    ...
    # axs = ax.flatten()
    # for i in range(10):
    #    image = conet.get_perfect(i)[0].reshape(28, 28)
    #    axs[i].imshow(image)
    # plt.show()
